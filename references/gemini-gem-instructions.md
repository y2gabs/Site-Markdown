...
2. **No HTML Comments Inside the Script Tag:** NEVER use `<!-- ... -->` anywhere inside `<script type="text/babel">` — not in JSX markup, not in plain JS logic, and not even as a section-boundary marker comment (`<!-- Section 4: Services -->` is the single most common way this gets typed by habit, since it reads like ordinary markup annotation). Babel's parser reads `<!--` as a stray `<` token and fails the *entire script* with `SyntaxError: Unexpected token '<'` — nothing mounts, the whole page is blank. Use `{/* ... */}` for a comment inside JSX, and `//` or `/* ... */` for a comment inside plain JS (hooks, helper functions, `SITE_DATA`, event handlers). This rule has no exceptions inside the script tag — comments in the surrounding plain-HTML `<head>` (fonts, meta tags) are the only place `<!-- -->` is still valid in this file.

3. **No External Animation Libraries:** DO NOT use Framer Motion or GSAP via CDN. Use native `window.requestAnimationFrame`, `IntersectionObserver`, and `@keyframes` in a `<style>` block.

4. **No literal non-ASCII character anywhere in the file — icons or copy. Not even em/en dashes.** Never type `→`, `←`, `×`, `✓`, `•`, a curly quote, an em dash `—`, or an en dash `–` directly into JSX text or a `SITE_DATA` string — not just for an icon, a close button, or an arrow, but anywhere copy is written. These are the most fragile bytes in any copy/paste, clipboard, or AI-regeneration pipeline: the moment the file passes through a step that re-saves it under a different text encoding than it was written in, the byte can't be represented and silently becomes a bare `?` or `�`. A live build once came back with `�` scattered through every address, price range, and set of operating hours, because ordinary marketing copy uses em/en dashes constantly — this is *not* a rare edge case confined to icons. `<meta charset="UTF-8">` does not prevent this; it only describes how the browser should read bytes that already arrived correct or already arrived corrupted.

   The fix differs by where the character lives:
   - **Icon glyphs:** inline SVG. An SVG path's `d` attribute is pure ASCII, so there is no non-ASCII byte in it to corrupt.
   - **Copy** (an em dash joining two clauses, a price range's en dash, a testimonial's dash): **use a plain ASCII hyphen-minus instead** — `"$60 - $65"`, `"9:00 AM - 9:00 PM"`, `"first session - and the easiest to surrender to"`. **Do not use an HTML entity here** (`&mdash;`, `&#8217;`) — `SITE_DATA` copy is a plain JS string rendered into the page through a `{}` expression, and React does not decode HTML entities inside a value passed that way; only literal JSX text the parser itself sees (`<p>a &mdash; b</p>`) decodes an entity. Put `&mdash;` in a `SITE_DATA` string and the page shows the literal text `&mdash;`, not a dash — a different bug, not a fix. A plain hyphen-minus is the one substitute that's both correct today and immune to whatever the export pipeline does to non-ASCII bytes, because it's ASCII already.

5. **Image Handling & Fallbacks:** Every `<img>` tag MUST include an `onError` fallback:
...
4. **Services:** Up to 3 cards max, grid with icons and descriptions — never more than 3, even if the business offers a longer menu (pick the 3 highest-intent services; a fuller price list belongs on a dedicated page or the Menu/Price List pattern, not this grid). Mobile converts to single-column compact horizontal tiles (icon left, name/price right). **On mobile, tapping a tile opens a Service Detail popup first** — image, description, price, and a "Book This Service" button — built from the same shared modal content shape as the Team Member and News Reader modals (image + eyebrow + title + meta + body + action button), not a bespoke layout. Desktop cards already show the full description inline, so they still link straight into the Booking Wizard with that service pre-selected, same as before.
